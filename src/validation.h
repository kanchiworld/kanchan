...
//! Discourage users to set fees higher than this amount (in photons) per kB
static const CAmount HIGH_TX_FEE_PER_KB = 0.01 * COIN;
//! -maxtxfee will warn if called with a higher fee than this amount (in photons)
 ...
 *  We have 4 times smaller block times in Kanchan, so we need to push 4 times more invs per 1MB. */
...
//static const int64_t DEFAULT_MAX_TIP_AGE = 6 * 60 * 60; // ~144 blocks behind -> 2 x fork detection time, was 24 * 60 * 60 in bitcoin //Gautam
static const int64_t DEFAULT_MAX_TIP_AGE = 60*60*24*365*20  ; // ~144 blocks behind -> 2 x fork detection time, was 24 * 60 * 60 in bitcoin
...
//static const bool DEFAULT_CHECKPOINTS_ENABLED = true; //Gautam
...
/** Absolute maximum transaction fee (in photons) used by wallet and mempool (rejects high fee in sendrawtransaction) */
