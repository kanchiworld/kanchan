...
{
    return QCoreApplication::translate("kanchan-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in kanchan.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in kanchan.qrc)
...
/** Class encapsulating Kanchan Core startup and shutdown.
...
/** Main Kanchan application object */
...
{
std::cout << "Entered BitcoinApplication::startThread " << std::endl;
    if(coreThread)
        return;
    ...
    coreThread->start();
std::cout << "Core thread started running" << std::endl;
}
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // kanchan: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Kanchan Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(kanchan);
    Q_INIT_RESOURCE(kanchan_locale);
    ...
    /// 6. Determine availability of data directory and parse kanchan.conf
    ...
    try
    {
        std::cout << "Entering SelectParams from main() " << std::endl;
        SelectParams(ChainNameFromCommandLine());
        std::cout << "Came out of SelectParams from main() " << std::endl;
    }
    ...
    // kanchan: links repeatedly have their payment requests routed to this process:
    ...
    {
#ifdef ENABLE_WALLET
    std::cout << "Wallet is enabled" << std::endl;
#else
    std::cout << "Wallet is NOT enabled" << std::endl;
#endif
        ...
        {
std::cout << "Going into app.requestInitialize() from main() " << std::endl;
            app.requestInitialize();
#if defined(Q_OS_WIN) && QT_VERSION >= 0x050000
            WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("%1 didn't yet exit safely...").arg(QObject::tr(PACKAGE_NAME)), (HWND)app.getMainWinId());
#endif
std::cout << "Going into app.exec() from main() " << std::endl;
            app.exec();
std::cout << "Came out of app.exec() from main() " << std::endl;
            app.requestShutdown();
            app.exec();
