...
{
    std::cout << "net.cpp: AdvertiseLocal" << std::endl;
...
{
    std::cout << "net.cpp: AddLocal" << addr.ToString() <<  std::endl;
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Kanchan users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Kanchan Core " + FormatFullVersion();
...
{
//std::cout << "Gautam: CConnman::ThreadMessageHandler()" << std::endl;
    while (!flagInterruptMsgProc)
    {
        ...
        for (CNode* pnode : vNodesCopy)
        {
            ...
            // Receive messages
//std::cout << "Gautam: CConnman::ThreadMessageHandler() m_msgproc->ProcessMessages " << std::endl;
...
//std::cout << "Gautam: CConnman::ThreadMessageHandler() SendMessages" << std::endl;
