...
const char * const BITCOIN_CONF_FILENAME = "kanchan.conf";
const char * const BITCOIN_PID_FILENAME = "kanchand.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KanchanCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KanchanCore
    // Mac: ~/Library/Application Support/KanchanCore
    // Unix: ~/.kanchancore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KanchanCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KanchanCore";
#else
    // Unix
    return pathRet / ".kanchancore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty kanchan.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override kanchan.conf
