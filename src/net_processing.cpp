...
static const int STALE_RELAY_AGE_LIMIT = 12* 30 * 24 * 60 * 60; //Gautam
...
static bool SendRejectsAndCheckIfBanned(CNode* pnode, CConnman* connman)
{
    ...
    {
        connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT,
                             (std::string)NetMsgType::BLOCK, reject.chRejectCode, reject.strRejectReason, reject.hashBlock));
    }
    state.rejects.clear();
